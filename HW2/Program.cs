namespace HW2
{
    internal class Program
    {
        static void Main(string[] args)
        {

            //Задание 1
            //Объявить одномерный (5 элементов) массив с именем A
            //и двумерный массив (3 строки, 4 столбца) дробных чисел с именем B.
            //Заполнить одномерный массив А числами, введенными с клавиатуры пользователем,
            //а двумерный массив В случайными числами с помощью циклов.
            //Вывести на экран значения массивов:
            //массива А в одну строку,
            //массива В — в виде матрицы.
            //Найти в данных массивах общий максимальный элемент,минимальный элемент,
            //общую сумму всех элементов, общее произведение всех элементов, сумму четных элементов массива А,
            //сумму нечетных столбцов массива В.

            Task1.Execute();

            //Задание 9
            //Напишите программу, которая принимает размеры двумерного массива
            //и число для умножения через параметры командной строки(например, 3 3 2).
            //Программа должна сгенерировать случайный двумерный массив,
            //умножить все его элементы на заданное число и вывести результат.

            //Task9.Execute();
        }
    }
}